...
void DisplayChannelInfos(HWND hW)
{
	...
	SetDlgItemInt(hW,IDC_CI7,s_chan[ch].iSilent,TRUE);
	...
	wsprintf(szB,"%-5x",(unsigned long)s_chan[ch].pStart-(unsigned long)spuMemC); SetDlgItemText(hW,IDC_CI10,szB);
	wsprintf(szB,"%-5x",(unsigned long)s_chan[ch].pCurr-(unsigned long)spuMemC); SetDlgItemText(hW,IDC_CI11,szB);
	wsprintf(szB,"%-5x",(unsigned long)s_chan[ch].pLoop-(unsigned long)spuMemC); SetDlgItemText(hW,IDC_CI12,szB);
	
	SetDlgItemInt(hW,IDC_CI13,s_chan[ch].iRightVolRaw,TRUE);
	SetDlgItemInt(hW,IDC_CI14,s_chan[ch].iLeftVolRaw,TRUE);
	//SetDlgItemInt(hW,IDC_CI15,s_chan[ch].iRawPitch,TRUE);
	SetDlgItemInt(hW,IDC_CI15,s_chan[ch].iUsedFreq,TRUE);
	SetDlgItemInt(hW,IDC_CI16, SoundGetBytesBuffered() / (output_samplesize * 45),TRUE);

	wsprintf(szB,"%-4x",s_chan[ch].iRightVolRaw); SetDlgItemText(hW,IDC_CI17,szB);
	wsprintf(szB,"%-4x",s_chan[ch].iLeftVolRaw); SetDlgItemText(hW,IDC_CI18,szB);
	...
	if(pSpuIrq==0) 
		...
		wsprintf(szB,"%5x",(unsigned long)pSpuIrq-(unsigned long)spuMemC);
		SetDlgItemText(hW,IDC_STA1,szB);
	}

	wsprintf(szB,"%04x",spuCtrl); SetDlgItemText(hW,IDC_STA2,szB);
	wsprintf(szB,"%04x",spuStat); SetDlgItemText(hW,IDC_STA3,szB);
	wsprintf(szB,"%-5x",spuAddr); SetDlgItemText(hW,IDC_STA4,szB);
	wsprintf(szB,"%-5x",rvb.StartAddr * 2); SetDlgItemText(hW,IDC_STA5,szB);
	...
	} else if( CDDAPlay != CDDAFeed ) {
		if(CDDAPlay<=CDDAFeed) ch=CDDAFeed-CDDAPlay;
		else               ch=(CDDAFeed-CDDAStart)+(CDDAEnd-CDDAPlay);
		SetDlgItemInt(hW,IDC_XA4,ch,FALSE);
		SetDlgItemInt(hW,IDC_XA5,iLeftXAVol,TRUE);
		SetDlgItemInt(hW,IDC_XA6,iRightXAVol,TRUE);
		SetDlgItemInt(hW,IDC_XA1,44100,TRUE);
		SetDlgItemInt(hW,IDC_XA2,44100,TRUE);
		SetDlgItemInt(hW,IDC_XA3,0,TRUE);
	} else {
		SetDlgItemInt(hW,IDC_XA4,0,FALSE);
	}


	wsprintf(szB,"%-4x",iVolMainL); SetDlgItemText(hW,IDC_MVOL1,szB);
	wsprintf(szB,"%-4x",iVolMainR); SetDlgItemText(hW,IDC_MVOL2,szB);
}
	...
	switch(uMsg)
  {
	...
	case WM_COMMAND:
    {
			...
			if(wParam>=IDC_MUTE1 && wParam<=IDC_MUTE24)       // mute clicked?
      {
			...
			if(wParam == IDC_RAM_DUMP ) {
				fp = fopen( "spu-ram.bin", "wb" );
				if( fp ) {
					fwrite( spuMemC, 1, 0x80000, fp );
					fclose(fp);
				}
			}
			
			if(wParam == IDC_RAM_LOAD ) {
				fp = fopen( "spu-ram.bin", "rb" );
				if( fp ) {
					fread( spuMemC, 1, 0x80000, fp );
					fclose(fp);
				}
			}
