...
static int RateTable_denom[128];
...
{    
	...
	int EnvelopeVol = ch->ADSRX.EnvelopeVol;
	int EnvelopeVol_f = ch->ADSRX.EnvelopeVol_f;
  ...
  {                                                    // do release
		...
		if( EnvelopeVol == 0 && EnvelopeVol_f == 0 ) {
			return 0;
		}


		rate = ch->ADSRX.ReleaseRate * 4;
		...
		if( EnvelopeVol_f >= RateTable_denom[ rate ] ) {
			...
			if(ch->ADSRX.ReleaseModeExp) {
				EnvelopeVol += ( RateTableSub[ rate ] * EnvelopeVol ) >> 15;
			}
			else
				EnvelopeVol += RateTableSub[ rate ];
		}
		...
		if(EnvelopeVol <= 0) 
    {
			...
			- notaz: allow channel to keep playing (IRQ generation)
			*/
			//ch->bOn=0;
			ch->iSilent = 2;
		}
		...
		ch->ADSRX.EnvelopeVol_f=EnvelopeVol_f;
		ch->ADSRX.lVolume=EnvelopeVol>>0;
		return EnvelopeVol>>0;
  }
	else                                                  // not stopped yet?
  {
		if(ch->ADSRX.State==0)                       // -> attack
    {
			rate = ch->ADSRX.AttackRate;

			if(ch->ADSRX.AttackModeExp && EnvelopeVol>=0x6000)
				rate += 8;
			...
			//if( spu_ch+1 >= 27 )
			fprintf( fp_spu_log, "ADSR-A %d = %X [%X] [%X]\n", spu_ch+1, EnvelopeVol, ch->ADSRX.AttackRate, ch->ADSRX.SustainLevel );
#endif
		...
		if(ch->ADSRX.State==1)                       // -> decay
    {
			...
			//if( spu_ch+1 >= 27 )
			fprintf( fp_spu_log, "ADSR-D %d = %X [%X] [%X]\n", spu_ch+1, EnvelopeVol, ch->ADSRX.DecayRate, ch->ADSRX.SustainLevel );
#endif



			// FF7 cursor - use Neill's 4-bit accuracy
			if( ((EnvelopeVol>>11)&0xf) <= ch->ADSRX.SustainLevel)
      {
				ch->ADSRX.State=2;
		...
		if(ch->ADSRX.State==2)                       // -> sustain
    {
      ...
      {
				// OPTIMIZE: fast finish
				if( EnvelopeVol == 0x7fff && EnvelopeVol_f == 0 )
					return 0x7fff;


				rate = ch->ADSRX.SustainRate;

				if( ch->ADSRX.SustainModeExp && EnvelopeVol>=0x6000 )
					rate += 8;
			...
			else
      {
			...
			//if( spu_ch+1 >= 27 )
			fprintf( fp_spu_log, "ADSR-S %d = %X [%X] [%X]\n", spu_ch+1, EnvelopeVol, ch->ADSRX.SustainRate, ch->ADSRX.SustainLevel );
#endif
